...
if (-not (Test-Path $python)) { throw "Run install.bat before building the backend." }
...
try {
    & $python -m PyInstaller --noconfirm --clean --distpath $Output --workpath $work --specpath $root `
        (Join-Path $root "1bullet-backend.spec")
    if ($LASTEXITCODE -ne 0) { throw "PyInstaller failed." }
    $exe = Join-Path $Output "1bullet-backend.exe"
    if (-not (Test-Path $exe)) { throw "PyInstaller did not create 1bullet-backend.exe." }
} finally {
